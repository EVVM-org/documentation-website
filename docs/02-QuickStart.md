...
- **Are you a Docker user?** Skip the local tooling setup entirely — the **[EVVM Docker Quick Start](./07-LibrariesAndTools/03-EvvmDocker/02-quickstart.md)** provides a pre-built image with everything ready to run. Keep in mind that the Docker setup deploys a standard EVVM with default parameters. If you need to customize contracts or deployment settings to fit your specific requirements, follow this guide instead.
:::
