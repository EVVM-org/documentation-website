...
The `SyncNonce` abstract contract provides sequential (synchronous) nonce management for EVVM services. Unlike async nonces, sync nonces must be used in strict sequential order (0, 1, 2, 3, ...).

## Overview

**Contract Type**: Abstract contract  
**License**: EVVM-NONCOMMERCIAL-1.0  
**Import Path**: `@evvm/testnet-contracts/library/utils/nonces/SyncNonce.sol`
...
abstract contract SyncNonce {
    ...
    function verifySyncNonce(address user, uint256 nonce) internal view virtual {
        if (syncNonce[user] != nonce) revert SyncNonceMismatch();
    }

    function getNextCurrentSyncNonce(address user) public view virtual returns (uint256) {
...
**Increment**: Only via `incrementSyncNonce()`
...
uint256 nextNonce = getNextCurrentSyncNonce(userAddress);
// nextNonce = 0 (first transaction)

// After first transaction processed
nextNonce = getNextCurrentSyncNonce(userAddress);
...
```typescript
// Query next nonce
const nextNonce = await contract.getNextCurrentSyncNonce(userAddress);
...
**Description**: Verifies that the provided nonce matches the expected next nonce

**Parameters**:
- `user`: Address to verify nonce for
- `nonce`: Nonce value to verify

**Visibility**: `internal view` - read-only check

**Reverts**: `SyncNonceMismatch()` if nonce does not match expected value

**Example**:
```solidity
function processAction(address user, uint256 nonce, ...) external {
    // Verify nonce matches expected (reverts if mismatch)
    verifySyncNonce(user, nonce);
